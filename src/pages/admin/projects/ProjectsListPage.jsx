...
const STATUS_LABEL = {
  PLANNED: { text: "계획", className: "bg-blue-100 text-blue-700" },
  ONGOING: { text: "진행중", className: "bg-emerald-100 text-emerald-700" },
  COMPLETED: { text: "완료", className: "bg-black/10 text-black/55" },
  SUSPENDED: { text: "보류", className: "bg-amber-100 text-amber-700" },
};
...
export default function ProjectsListPage() {
  ...
  const handleDelete = async (id, title) => {
    ...
    try { await projectsApi.remove(id); reload(); }
  ...
  return (
    <div>
      <PageHeader
        title="Projects"
        description="연구 과제(프로젝트)를 관리합니다."
        actions={
          <button
            type="button"
              ...
              <table className="w-full text-[13px]">
                <thead className="bg-black/[0.04] text-left text-[12px] font-semibold uppercase tracking-wide text-black/65">
                  <tr>
                    ...
                    <th className="w-[160px] px-4 py-3">발주기관</th>
                    <th className="w-[180px] px-4 py-3">기간</th>
                ...
                <tbody className="divide-y divide-black/5">
                  {items.length === 0 ? (
                    <tr><td colSpan={7}><EmptyState title={keyword ? "검색 결과가 없습니다." : "아직 등록된 프로젝트가 없습니다."} /></td></tr>
                  ) : items.map((it) => {
                    ...
                    return (
                      <tr key={it.id} className="hover:bg-black/[0.02]">
                        <td className="px-4 py-3">
                          {it.thumbnailImage ? (
                            <img src={it.thumbnailImage} alt={it.title} className="h-12 w-16 rounded object-cover" />
                          ) : (
                            <div className="h-12 w-16 rounded bg-black/5" />
                          )}
                        </td>
                        <td className="px-4 py-3">
                          <Link to={`/admin/projects/${it.id}`} className="font-medium hover:underline">{it.title}</Link>
                          {it.role && <div className="text-[12px] text-black/55">{it.role}</div>}
                        </td>
                        <td className="px-4 py-3 text-black/70">{it.fundingAgency || "—"}</td>
                        <td className="px-4 py-3 text-black/65 text-[12px]">
                          {it.startDate} ~ {it.endDate || "진행중"}
                        </td>
                        <td className="px-4 py-3">
